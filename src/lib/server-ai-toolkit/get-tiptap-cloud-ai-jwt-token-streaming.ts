...
/**
 * Generates an ES256-signed JWT for authenticating Server AI Toolkit streaming
 * calls via the Tiptap Access Control flow.
 *
 * Requires:
 ...
 * tools) and `Documents:Read` + `Documents:Write` (so it can open a Hocuspocus
 * session on the user's behalf with the same JWT).
 */
export function getTiptapCloudAiJwtTokenStreaming(): string {
